...
def _load_supabase_integrations():
    try:
        from .repository.supabase_client import is_configured, select_one

        if not is_configured():
            return None
        row = select_one("settings", {"chave": "integrations"})
        if row and isinstance(row.get("valor"), list):
            return [normalize_integration(item) for item in row["valor"]]
    except Exception:
        return None
    return None


def _save_supabase_integrations(items):
    try:
        ...
        upsert_row(
            "settings",
            {"chave": "integrations", "valor": items, "descricao": "Integracoes preparatorias Website"},
            on_conflict="chave",
        )
    except Exception:
        pass
...
def ensure_integrations_loaded(app):
    if hasattr(app, "integrations"):
        ...
        return
    remote = _load_supabase_integrations()
    if remote is not None:
        app.integrations = remote
        return
    app.integrations = []
...
def save_integrations(app):
    ...
    _save_supabase_integrations(app.integrations)
