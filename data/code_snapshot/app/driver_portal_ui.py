...
def render_driver_portal_page(app, driver, slug):
    ...
    html = html.replace("__CPF_HINT__", "")
...
.boot-shell{min-height:100vh;display:grid;place-items:center;background:#111827;color:#e5e7eb}.boot-card{width:min(420px,calc(100% - 40px));background:#172033;border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:24px;text-align:center;box-shadow:0 22px 60px rgba(0,0,0,.28)}.boot-title{font-size:1.05rem;font-weight:800;text-transform:uppercase;margin-bottom:8px}.boot-line{height:4px;border-radius:999px;background:linear-gradient(90deg,var(--primary),#93c5fd);margin-top:16px;animation:pulse 1.1s ease-in-out infinite}@keyframes pulse{0%,100%{opacity:.45}50%{opacity:1}}
...
<body>
<div class="boot-shell" id="bootShell"><div class="boot-card"><div class="boot-title">Portal Motorista</div><div>Validando sessao segura...</div><div class="boot-line"></div></div></div>
<div class="auth-shell hidden" id="authShell">
  <section id="view-login" class="login-wrap">
    <div class="auth-card">
      ...
      <input id="cpf" inputmode="numeric" autocomplete="username" value="__CPF_HINT__"/>
      <label for="password">Senha</label>
      <input id="password" type="password" autocomplete="current-password" placeholder="Senha do portal ou token de ativacao"/>
  ...
  <section id="view-set-password" class="login-wrap hidden">
    <div class="auth-card">
      ...
      <label>Nova senha</label><input id="newPassword" type="password" autocomplete="new-password"/>
      <label>Repita a senha</label><input id="newPassword2" type="password" autocomplete="new-password"/>
...
function showAuth(name){hide($('bootShell'));hide($('appShell'));show($('authShell'));['login','set-password'].forEach(v=>hide($('view-'+v)));show($('view-'+name))}
function setView(name){hide($('bootShell'));hide($('authShell'));show($('appShell'));['dashboard','reservations','reservation-form','detail','clients','finance','notifications','profile','settings'].forEach(v=>hide($('view-'+v)));show($('view-'+name));$('pageTitle').textContent=VIEW_TITLES[name]||'Portal';document.querySelectorAll('#sideNav button').forEach(b=>b.classList.toggle('active',b.dataset.view===name));if(name==='reservations')loadReservations();if(name==='clients')loadClients();if(name==='finance')loadFinance();if(name==='settings')loadSettings();if(name==='notifications')loadNotifications();if(name==='profile')loadProfile()}
...
async function bootApp(){if(!token){showAuth('login');return}if(sessionStorage.getItem('driver_pending_password')==='1'){showAuth('set-password');return}let dash;try{dash=await api('/api/driver/dashboard')}catch(e){dash={ok:false}}if(!dash.ok){token='';sessionStorage.clear();showAuth('login');return}renderDashboard(dash);await Promise.all([loadReservations(),loadClients(),loadSettings(),loadProfile(),loadNotifications(),loadFinance()]);setView('dashboard')}
